...
- **P0 实际剩余缺口 ≈ 0。**
- **P1 后续执行记录（同日）**：
  - **M2 命令面板已落地**：新增 `CommandPalette.vue`，`⌘Shift+K / Ctrl+Shift+K` 唤起，可模糊搜索跳转 14 个页面 + 高频动作（问 AI 副驾 / 切换主题），↑↓ 选择 / Enter 执行 / Esc 关闭；保留原 `⌘K` 开副驾习惯不变。纯前端，已部署生产。
  - **M4 Forecast.vue 巨石拆分 = 评估后暂不拆**：6564 行中 P11-P16 增强均真实接线（非死代码），Excel 导入 / AI 审核台 / 返利看板等子模块深度读写共享状态（`cross.rows`/`cross.period`/`rebatePushList`），无「真正独立、低风险可抽」的边界。一次性大拆 = 8-13 人日 + 全链路回归、零直接可感知收益，与「运维成本不可控 / 生产稳定优先」红线相悖。**改渐进式**：将来具体改到某功能时顺带抽那一个子组件（一次一个、改哪抽哪）。
- **剩余 P1/P2 建议**：M1 记忆分层、M3 多模型、M5 Adapter 去桩，按业务价值择机推进；通用平台能力（多模态 / 代码沙箱 / 专家市场 / 追 Hermes 大版本）一律不追。
