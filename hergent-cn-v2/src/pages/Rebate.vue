<template>
  <div class="page">
    ...
    <Teleport to="body">
      ...
      <Transition name="modal">
        <div v-if="showForm" class="modal-card" @keydown.enter="onFormEnter">
          <div class="modal-hd">
            <b>{{ editing ? '编辑规则' : '新建规则' }}</b>
            <button class="btn-close" @click="showForm=false"><Icon name="close"/></button>
          </div>
          <div class="modal-body">
            <div class="form-row"><label>规则名称</label><input ref="formNameRef" v-model="form.rule_name" class="input" placeholder="如 蒙牛低温8月目标"></div>
            <div class="form-grid2">
              ...
              <div class="form-row" v-if="form.dimension!=='brand'"><label>目标度量</label>
            ...
            <div v-if="isBrandForm" class="monthly-block" style="margin:8px 0 4px;border:1px solid var(--bd);border-radius:10px;padding:10px 12px;background:rgba(var(--p-rgb),.04)">
              <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap">
                <b>品牌目标</b>
                <div class="seg">
                  <button type="button" :class="['seg-btn', brandMode==='year'?'on':'']" @click="switchBrandMode('year')">年度（按 12 个月分解）</button>
                  <button type="button" :class="['seg-btn', brandMode==='single'?'on':'']" @click="switchBrandMode('single')">单期（月，单值）</button>
                </div>
              ...
              <div class="form-grid3" style="margin-top:8px">
                ...
                <div class="form-row"><label>全年目标</label>
                  <div class="input-affix"><input v-model.number="annualTargetWan" class="input" type="number" min="0" step="0.1" placeholder="填全年，自动均分到 12 个月"><span class="affix">万元</span></div>
                </div>
                <div class="form-row"><label>全年返利率</label>
                  <div class="input-affix"><input v-model.number="annualRatePct" class="input" type="number" min="0" max="100" step="0.1" :placeholder="annualRatePlaceholder"><span class="affix">%</span></div>
                </div>
              </div>
              <div class="monthly-grid" style="margin-top:8px">
                <div class="monthly-grid-hd" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
                  ...
                  <span class="page-sub">已填 <b>{{ monthlyFilledCount }}</b> 个月 · 合计 <b>{{ monthlySumWan }}</b> 万元</span>
              ...
              <div class="form-grid2" style="margin-top:8px">
                ...
                <div class="form-row"><label>本期目标金额</label>
                  <div class="input-affix"><input v-model.number="targetWan" class="input" type="number" min="0" step="0.1" placeholder="如 100"><span class="affix">万元</span></div>
                </div>
            ...
            <!-- 非品牌（商品目标 / 数量度量）：仍按单值目标录入 -->
            <div v-else class="form-grid2">
              ...
              <div class="form-row" v-if="form.target_type==='amount'"><label>目标金额</label>
                <div class="input-affix"><input v-model.number="targetWan" class="input" type="number" min="0" step="0.1" placeholder="如 16"><span class="affix">万元</span></div>
              </div>
                ...
                <table class="dt-tier" style="margin-top:8px">
                  ...
                  <tbody>
                    <tr v-for="(t,i) in ruleTiers" :key="i">
                      ...
                      <td><button class="btn btn-ghost btn-xs danger" type="button" @click="ruleTiers.splice(i,1)">删除</button></td>
                    </tr>
                  </tbody>
                </table>
            ...
            <!-- v122：报单节奏是品牌方（蒙牛/简爱）的排产节点，与单品无关 —— 商品维度整块隐藏，
                 不再逼用户给一个单品填「报单日 / 到货日 / 智能建表」 -->
            <div v-if="isBrandForm" class="arrival-block">
              ...
              <div class="form-grid3">
                <div class="form-row"><label>提前天数</label>
                  <div class="input-affix"><input :value="form.order_lead_days ?? ''" class="input" type="number" min="0" max="30" step="1" @input="onLeadInput"><span class="affix">天到货</span></div>
                </div>
                <div class="form-row"><label>首次到货日<span class="ap-sec-note">自动算，可改</span></label>
                  <input :value="firstArrivalDate" class="input" type="date" @change="onArrivalChange">
                </div>
              ...
              <p v-if="arrivalPreview && arrivalPreview.source==='arrival'" class="ap-warn">
                ...
                <button type="button" class="ap-adopt" @click="openMigrate">从旧排程反推</button>
              </p>
...
const ruleTiers = ref([])
// 阶段2: 品牌目标两层结构 —— monthlyRows 每行 {mm:'01'..'12', amtWan, ratePct}
// v121c：取消「是否按月分解」开关 —— 品牌目标固定就是 12 个月分解，进页面直接铺开
const MONTHS_12 = ['01','02','03','04','05','06','07','08','09','10','11','12']
const monthlyRows = ref([])
const legacyNotice = ref('')   // 老规则口径不同、未自动拆分时的提示
// v122 (R5-A)：品牌目标两种模式 —— 'year'=年度（按 12 个月分解）；'single'=单期（月，单值）
//   存量 tenant_1 的 4 条真实规则全是 month 口径，若强制年度会把「月目标 100 万」变成「年目标 100 万」，
//   返利基数少算 12 倍。故支持双模式，按规则自身 period_type 回填，不擅自改口径。
const brandMode = ref('year')
// 品牌维度（品牌目标只能按金额，后端亦如此约束）
const isBrandForm = computed(() => !!form.value && form.value.dimension === 'brand')
// 品牌维度 + 金额度量 → 可走 12 个月分解（后端亦只允许品牌维度用 monthly_amounts/monthly_rates）
const isBrandMonthly = computed(() => !!form.value && form.value.dimension === 'brand' && form.value.target_type === 'amount')
// 当前是否处于「12 个月分解」录入模式 = 品牌 + 年度模式
const monthlyOn = computed(() => isBrandMonthly.value && brandMode.value === 'year')
// 年度模式下「目标年度」必填；默认当前年
const currentYear = new Date().getFullYear()
// 月合计（万元）—— 年度总额即月合计，金额层单一真相
const monthlySumWan = computed(() => {
  const s = monthlyRows.value.reduce((a, r) => a + (Number(r.amtWan) || 0), 0)
  return Math.round(s * 100) / 100
})
// 全年一次性录入：读 = 月合计；写 = 均分到 12 个月（余数补 12 月）
const annualTargetWan = computed({
  get: () => (monthlySumWan.value > 0 ? monthlySumWan.value : ''),
  set: (v) => splitAnnualToMonths(v),
})
// 全年返利率：读 = 12 个月一致时显示该值，否则留空（placeholder 提示「各月不同」）；写 = 全月统一
const annualRatePct = computed({
  get: () => {
    ...
    const first = vals[0]
    return vals.every(v => Math.abs(v - first) < 1e-9) ? first : ''
  },
  set: (v) => {
...
// 已填月份数 & 「全年返利率」的占位提示（各月不一致时提示"各月不同"，避免误以为没填）
...
function splitAnnualToMonths(inputWan) {
  const totalWan = Number(inputWan != null ? inputWan : targetWan.value) || 0
  monthlyRows.value.forEach(r => { r.amtWan = null })   // 只重算金额，不动返利率
...
// v122 (R5-A)：年度（12 个月分解）⇄ 单期（月，单值）切换。
//   不清空 monthlyRows —— 切回年度模式数据还在，避免误触丢数据；保存时才按当前模式决定写不写月度分解。
function switchBrandMode(m) {
  ...
  if (m === 'year') {
    ...
    // 原「年」口径切到单期 → 降为「月」，避免保存后仍是年口径但只有一个单值
...
function fillMonthlyFromRule(r) {
  ...
  // v122 (R5-A)：模式回填 —— 有月度分解 或 本身即年口径 → 年度模式；否则单期模式（保留原 period_type，不改口径）
  brandMode.value = (hasMonthly || r.period_type === 'year') ? 'year' : 'single'
  ...
  // 年度模式：老「年口径无月分解」规则直接均分预填，值不丢；单期模式保持单值，不做任何改写
  if (brandMode.value === 'year' && !hasMonthly && isBrandMonthly.value && tvWan > 0) {
    splitAnnualToMonths(tvWan)
  }
}
...
// v112 R18：编辑/新建弹窗自动聚焦首个字段
const formNameRef = ref(null)

// v112 R18：表单内回车触发保存（按钮聚焦时回车由浏览器触发 click，跳过防双触发）
...
// v118 (L2细化)：到货排程引擎配置 —— 目标以万元录入、双模式、智能预填、均单实时试算
...
// v121 合并后到货星期改由报单星期派生，原 isWk/toggleWk（到货语义）已移除，勿再补回
function fmtWan(n) {
  const v = Number(n) || 0
  return v >= 100 ? Math.round(v).toLocaleString() : (Math.round(v * 100) / 100).toString()
}

// ---------- v121b：报单日 ↔ 提前天数(N) ↔ 到货日 三字段双向联动 ----------
// 数据结构约定：到货日【不落库】，落库的只有 N(order_lead_days)。
//   理由：到货日若单独存列，就会与「报单日 + N」形成双源，改一边必产生矛盾数据。
//   因此到货日 = 派生展示值，用户手改时立即反推成 N 写回，任何时候都只有一个真相。
const LEAD_MIN = 0, LEAD_MAX = 30
const WD_CN = { 0: '日', 1: '一', 2: '二', 3: '三', 4: '四', 5: '五', 6: '六' }
// 解析 YYYY-MM-DD；用 Date.UTC 计算，跨月/跨年/闰年(2024-02-29)自动正确，且不受本地时区影响
function _parseISO(s) {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(s == null ? '' : s).trim())
  if (!m) return null
  const y = +m[1], mo = +m[2], d = +m[3]
  if (mo < 1 || mo > 12 || d < 1) return null
  const last = new Date(Date.UTC(y, mo, 0)).getUTCDate()   // 该月最后一天，自动含闰年
  if (d > last) return null
  return { y, mo, d }
}
function _addDaysISO(s, n) {
  const p = _parseISO(s); if (!p) return ''
  const dt = new Date(Date.UTC(p.y, p.mo - 1, p.d) + Math.trunc(n) * 86400000)
  return dt.getUTCFullYear() + '-' + String(dt.getUTCMonth() + 1).padStart(2, '0') + '-' + String(dt.getUTCDate()).padStart(2, '0')
}
function _diffDaysISO(a, b) {
  const pa = _parseISO(a), pb = _parseISO(b); if (!pa || !pb) return null
  return Math.round((Date.UTC(pb.y, pb.mo - 1, pb.d) - Date.UTC(pa.y, pa.mo - 1, pa.d)) / 86400000)
}
function _weekdayOf(s) {
  const p = _parseISO(s); if (!p) return ''
...
// 到货日：报单日为空 或 N 非法/清空 → 一律返回空，不残留旧值
const firstArrivalDate = computed(() => {
  const f = form.value
  if (!f.order_first_date) return ''
  if (!leadValid.value) return ''
...
function onLeadInput(e) {
  ...
  // 清空 N → 到货日同步清空（不回落默认值，避免"看着空、实际按 4 天算"）
...
function onArrivalChange(e) {
  ...
  if (!v) { f.order_lead_days = null; return }              // 清空到货日 → N 同步清空
  if (!f.order_first_date) {                                 // 无报单日无法反推
  ...
  f.order_lead_days = diff                                   // 反推回填 N，到货日随之显示新值
}
let _apTimer = null
async function loadArrivalPreview() {
  const f = form.value
  // v121c：品牌目标的年度总额 = 12 个月之和（不再有独立的「目标金额」框）
  ...
  // v121 合并：优先透传【报单语义】—— 到货日由报单日派生，与智能报单表预览同源
  params.set('order_mode', f.order_mode || 'interval')
  // v121b：报单日 与 N 两者都有效才走报单口径；任一为空/非法 → 回退旧到货口径，
  //        与「到货日」联动保持一致（避免页面显示空、后台却按 4 天算）
  ...
  try {
    ...
    // api() 已解包信封，r 即为 summary 载荷（含 count/per_order_wan/unit），无 .data 层
...
async function loadAutoPeriodPreview() {
  ...
  // v121b：N 非法/清空 -> 到货日算不出来，预览不展示（上方 leadErr 已给出原因）
...
// 报单星期多选（chips 交互；到货星期已由报单星期 + 提前天数派生，不再单独配置）
...
// ---------- v121 合并：老规则「到货排程(arrival_*)」→「报单节奏(order_*)」反推迁移 ----------
const WK_LABEL = { 1: '一', 2: '二', 3: '三', 4: '四', 5: '五', 6: '六', 7: '日' }
const showMigrate = ref(false)
const migrateInfo = ref(null)
function _isoLocal(d) {
  return d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0')
}
// 打开反推弹窗：由旧到货语义推出报单语义（到货日 = 报单日 + lead）
function openMigrate() {
  ...
  if (mode === 'weekday') {
    ...
    migrateInfo.value = {
      ...
      to: { mode: 'weekday', first_date: _isoLocal(new Date()), cadence: null, weekdays: owd.join(',') },
    }
  } else {
    ...
    migrateInfo.value = {
      ...
      toLabel: '报单：从 ' + _isoLocal(d) + ' 起、每 ' + cad + ' 天一次（到货 = 报单 + ' + lead + ' 天）',
      to: { mode: 'interval', first_date: _isoLocal(d), cadence: cad, weekdays: '' },
    }
  }
...
// 表单相关字段变化时实时重算均单（防抖）
// v121 合并：到货次数改由报单节奏派生，故 order_* 变化也必须重算均单
watch(
  () => [form.value.arrival_mode, form.value.arrival_first_dom, form.value.arrival_cadence_days,
         form.value.arrival_weekdays, form.value.arrival_count_override, form.value.target_type, targetWan.value, form.value.target_value,
         // v121c：品牌目标改由 12 个月之和驱动均单，月份变动也要重算
         monthlySumWan.value,
         form.value.order_mode, form.value.order_first_date, form.value.order_cadence_days,
         form.value.order_weekdays, form.value.order_lead_days],
  () => {
...
// 作用对象候选：按维度切换 datalist 内容（渠道用 select 单独渲染）
const scopeDatalist = computed(() => {
  const d = form.value.dimension
  if (d === 'brand') return brandOptions.value
  // v122：商品维度给真实商品名候选（之前返回空，用户只能手填 ID）
  if (d === 'product') return productNames.value
  return []
})
...
function openCreate(presetDim) {
  editing.value = false
  form.value = {
    // v122：商品目标多数按金额 → 商品/品牌一律默认 amount（数量可手动切）
    rule_name: '', dimension: presetDim || 'brand', target_type: 'amount',
    scope_key: '', scope_name: '', period_type: presetDim === 'brand' ? 'year' : 'month',
    target_value: 0, trigger_mode: 'on_target', trigger_threshold: 1,
    // v122：目标年度（12 个月分解必须标明是哪一年）+ 数量单位（箱/件/提）
  ...
  ruleTiers.value = []
  legacyNotice.value = ''
  brandMode.value = 'year'      // v122：新建默认年度模式（12 个月分解）
  resetMonthlyRows()          // v121c：品牌目标默认铺开 12 个月分解（空表，等用户填全年或单月）
  showForm.value = true
...
function openEdit(r) {
  editing.value = true
  form.value = { ...r }
  // v122：老规则无 target_year → 默认当前年（年度模式下用户可见、可改）；target_unit 缺省空
...
// v112 R11：复制规则 —— 复制全部配置，清空生效期与 id（走新建），名称加「副本」后缀便于区分
function dup(r) {
  editing.value = false
  form.value = {
...
async function save() {
  const f = form.value
  // v118/v121c：金额目标以「万元」录入，保存时转回「元」(target_value 单位不变，rebate_calc 口径不动)
  // 品牌目标走 12 个月分解 → 年度总额 = 各月之和；其余（商品/数量）沿用单值录入
  if (isBrandMonthly.value && brandMode.value === 'year') {
  ...
  if (isBrandMonthly.value && brandMode.value === 'year') {
    // v121c：缺目标/缺返利直接在前端拦住（后端会 400，别让用户撞墙）
  ...
  // v121 合并：UI 已不再编辑 arrival_*，改校验报单语义（未迁移规则 order_mode 默认 interval，不会误伤）
  if (f.order_mode === 'weekday' && !f.order_weekdays) { toast('请选择至少一个报单星期', 'error'); return }
  // v121b：报单日 / N / 到货日 一致性 —— 填了报单日但 N 非法，等于存一条"到货日算不出来"的半成品
  ...
  // v122：商品维度填了名字必须解析成商品 ID —— 否则 scope_key 为空 = 作用于全部单品，与用户意图不符
  if (f.dimension === 'product' && f.scope_name) {
    f.scope_key = resolveProductKey(f.scope_name)
  }
