      ...
      <Transition name="modal">
        <div v-if="showForm" class="modal-card" @keydown.enter="onFormEnter">
          ...
          <div class="modal-body">
            ...
            <!-- v118 (L2细化)：到货排程引擎 —— 双模式(间隔天数/固定星期) + 首次到货日 + 本月次数(可覆盖) + 均单 -->
            <div class="arrival-block">
              <div class="form-row"><label>到货模式</label>
                <div class="seg">
                  <button type="button" :class="['seg-btn', form.arrival_mode==='interval'?'on':'']" @click="form.arrival_mode='interval'">按间隔天数</button>
                  <button type="button" :class="['seg-btn', form.arrival_mode==='weekday'?'on':'']" @click="form.arrival_mode='weekday'">按固定星期</button>
                </div>
              </div>
              <div class="form-grid2" v-if="form.arrival_mode==='interval'">
                <div class="form-row"><label>首次到货日</label><input v-model.number="form.arrival_first_dom" class="input" type="number" min="1" max="31" placeholder="月内日 1-31"></div>
                <div class="form-row"><label>到货周期(天)</label><input v-model.number="form.arrival_cadence_days" class="input" type="number" min="1" placeholder="如 2 = 每2天到货"></div>
              </div>
              <div class="form-row" v-else>
                <label>到货星期</label>
                <div class="wk-chips">
                  <button v-for="w in weekdayOptions" :key="w.value" type="button" :class="['wk-chip', isWk(w.value)?'on':'']" @click="toggleWk(w.value)">{{ w.label }}</button>
                </div>
              </div>
              <div class="form-grid2">
...
function isWk(v) {
  return String(form.value.arrival_weekdays || '').split(',').map(s => s.trim()).filter(Boolean).includes(String(v))
}
function toggleWk(v) {
  const cur = String(form.value.arrival_weekdays || '').split(',').map(s => s.trim()).filter(Boolean)
  const i = cur.indexOf(String(v))
  if (i >= 0) cur.splice(i, 1); else cur.push(String(v))
  form.value.arrival_weekdays = cur.join(',')
  loadArrivalPreview()
}
function fmtWan(n) {
  const v = Number(n) || 0
...
async function loadArrivalPreview() {
  const f = form.value
  const tv = f.target_type === 'amount' ? (Number(targetWan.value) || 0) * 10000 : (Number(f.target_value) || 0)
...
async function save() {
  ...
  if (f.arrival_mode === 'weekday' && !f.arrival_weekdays) { toast('请选择至少一个到货星期', 'error'); return }
...
async function loadProductRefs() {
  try {
    ...
    for (const p of items) {
      ...
      if (p.name) byName.set(String(p.name).trim().toLowerCase(), pid)
