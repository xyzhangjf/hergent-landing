...
# ── 2026-09-24 傍晚：D5 被用户订正（加单列预填 = 均单剩余 − 本期报单合计）─────────
# 同一个文件**又**撞上「我的段 + 对方在途段被并进同一个 hunk」（§5.23 场景复现）：
#   `@@ -2191,2 +2191,7 @@` → + 侧 7 行 = 对方的 v248 段（5 行）+ 我改的标题/方案行（2 行）
#   —— 我的段上一次已进 HEAD，对方那 5 行仍是工作区在途；这次我改的标题行**正好落在
#   对方插入之后**，于是两个改动被 git 并成一个 hunk。
#   · 归属用 `own_hunks: [2191]`（正向认领），再 `trim_plus_head: {2191: 5}` 丢掉对方 5 行。
#   · 另外两个 hunk（2197 / 2202）纯属我：`-1/+2`（修正被证伪的判据）、`-4/+15`（D5 整段重写）。
#   ⚠️ 行号会随并发写入漂移 ⇒ 若对方又动了这段，工具有 `missing` 断言会当场报错，别改成宽松匹配。
SPEC_MEM_TARGET_0924B = ("fe", [
    {"file": ".workbuddy/memory/topics/forecast-order-domain.md",
     "own_hunks": [2191, 2197, 2202],
     "trim_plus_head": {2191: 5},
     "present": ["口径已**全部**拍板", "绝不能数期次表", "预填值(箱) =",
                 "循环依赖", "三套口径"],
     "gone": [],
     "dropped": ["## 🔴 客户列永不收（2026-09-22 v248 拍板）"]},
    # 本工具自身：新增上面这条 spec + 注册。
    {"file": ".workbuddy/tools/scoped_stage_by_marker.py", "keep_all": True, "gone": []},
])
