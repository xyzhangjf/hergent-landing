...
- **预报主表 · 期次 · 导入登记 · 到货周期** → `topics/forecast-order-domain.md`
   ...
   **条码重复判据 v178 → v196** · **品牌归并 v196**（福宝/恒滋 → 蒙牛低温）· 只发改动行 v196 ·
   ...
   🔴 **报单人门店配置收敛 v200**（`employee_stores` vs `report_mapping` **两表零交集** ⇒
   在报单配置里配了门店小程序看不到；**读端取并集**、报单配置成为真源、`employee_stores` 退化为
   **只读历史层** + `prune_covered` 收敛 ⇒ 停用即真收回。⚠️ `prune_covered(0)` = **清全租户**，调用点必判 0。
   ⚠️ 入口在 **预报订单管理 → 报单配置**（`/forecast` 内嵌，非独立路由），**不是** `设置›报单配置`）
