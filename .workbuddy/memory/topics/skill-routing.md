...
- `hergent-rebate-caliber-consistency` —— 口径不一致（分子分母不同源 / 归组键错配 / 阈值硬编码 /
  **单期 vs 累计（差额会恒 0）** / **跨表按名字匹配要先判「两侧同源」** / **配置项作用域（品牌≠全局）** /
  ⭐ **§13「之前已实现」≠可复用 —— 摊销 vs 归位**（两者差 `已报×(1−1/期次)`，期次=15 时差 93%）/
  **派生量循环依赖**（依赖了含自身输出的量 ⇒ 一填它自己就变小，零报错）/
  **配置值存在 ≠ 支撑它的基础数据存在**（`override=15` 而期次表只 3 行 ⇒ 别数那张表））
...
- `hergent-write-entry-gate` —— 给写入口加「前置条件门禁」（服务端硬拒 + 前端禁用并说明原因）
  · 🔴 **两个变体**：① **锁型门禁**（「审核后不可编辑」这类，判据 = 锁定态 `pid=0` 绝不能判）；
    ② **建议值型写入**（系统自动算出的数填进真实写库的格子 —— 预填/推荐/默认值/AI 填好的一栏）。
    建议值型：格子里有数 ≠ 是用户的意思 ⇒ **四态**（空 / ◐建议态不算 / ○已采纳 / ●已手改）
    + 保存前**一次**闸门【采用/忽略/再看看】+ 落库记 `source`；🔴 **建议值重算只刷"仍是建议态"的格**
    （否则用户改过的数被刷回去、零报错）+ **"忽略"必须明确落 0 = 不加也不减**。
    触发：预填 · 建议值 · 自动填 · 系统代填 · 默认值
