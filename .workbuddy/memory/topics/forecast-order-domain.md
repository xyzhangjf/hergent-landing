...
## §商品目标管理 · 口径已拍板 4/5（2026-09-24 · **仅方案，未动代码**）
- 方案文档：`outputs/商品目标管理-需求梳理与开发计划-2026-09-24/01-需求梳理与开发计划.md`（D1~D4 已落定，D5 待选落点）。**起号前重搜**（记忆里已见 v262 ⇒ 至少 v263 起）。
...
- **D2 可报单数**（拍板"取用户在系统里设置的数"）= **`rebate_target_rules.arrival_count_override`**（中文标签「**本月到货次数**」，`rebate_rules.py:157`）。
  ...
  剩余可报单数 = 设置值 − 本月已报期次数（9 月实测 15 − 3 = 12）。🔴 **实时读取、不读快照**（否则用户改了设置界面不跟着变）⇒ `product_targets.order_count` 只作审计，不参与计算。
- **D3 减单夹断**（拍板"不能扣到负数，最低到0"）：`alloc_i = max(D × ratio_i / 100, −该人本期已报箱数)`。
  ⚠️ 遗留子问题（待用户选）：夹断后**要不要把多出的部分再分给有余量的人**？推荐"要"（保"说减 60 就真减 60"，否则与加单总量栏长期对不上账）。
...
- **D5 差额合计 = 加单格的参考值**（用户原话不懂，已配 SVG 图解释）：
  `差额合计(箱) = Σ_承接人 max(0, 均单目标 − 该人**本期**实报)`，浅灰占位显示 + 悬停出逐人明细。
  🔴 **不要与「本月累计实报」比** —— 均单目标已是"这一单该报多少"，被减数必须同维度，否则差额恒 0（"人人早就超标"）。
  ⚠️ 它**只是参考值**：按占比分 ≠ 精确补齐（只有当占比 ∝ 缺口时才相等），经理可填更大/更小/负数。
- 🔴 **数据链（核实过的真路径，实现时照走）**：
