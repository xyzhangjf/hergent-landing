...
## §商品目标管理 · 口径已**全部**拍板（2026-09-24 · **仅方案，未动代码**）
- 方案文档：`outputs/商品目标管理-需求梳理与开发计划-2026-09-24/01-需求梳理与开发计划.md`（D1~D4 已落定；**D5 当日被用户订正**为「加单列预填 = 均单剩余 − 本期报单合计」）。**起号前重搜**（记忆里已见 v262 ⇒ 至少 v263 起）。
...
- **D5 ✅ 用户当日订正**（上一轮写的"Σ 各人 `max(0,…)`"**按人累加口径作废**）：加单列**预填**
  ...
  ⚠️ **待用户定**：预填值**算不算数**（建议态 + 保存前一次确认 / 直接生效）。后端 `save-matrix` 对 `extra_qty`
     **没有负数校验**（`eq = float(r.get("extra_qty") or 0)` 直落库）⇒ "预填即生效"会**静默产生加减单**，故推荐前者。
