  ...
  function switchPage(pageId) {
    ...
    if (pageId === 'pageTeamwork') renderTeamworkQuickGrid();
...
${questionnaireHistory}`;
  ...
  function renderTeamworkQuickGrid() {
    const grid = document.getElementById('twQuickGrid');
    if (!grid) return;
    grid.innerHTML = TEAMWORK_QUICK.map(t => {
      const agentTags = t.agents.map(a => {
        const role = ROLES[a];
        return `<span class="tw-qc-agent"><img src="avatar://${a}.png" onerror="this.style.display='none'" />${role ? role.name : a}</span>`;
      }).join('');
      return `<div class="tw-quick-card" onclick="fillTeamworkPrompt('${t.prompt.replace(/'/g, "\\'")}')">
        <div class="tw-qc-title">${t.title}</div>
        <div class="tw-qc-desc">${t.desc}</div>
        <div class="tw-qc-agents">${agentTags}</div>
      </div>`;
    }).join('');
  }

  function fillTeamworkPrompt(prompt) {
  ...
  function startTeamwork() {
    ...
    if (!task) { showDialog('⚠️', '说说你想做什么'); return; }

    // 停止之前的动画
    if (_twTimer) { clearInterval(_twTimer); _twTimer = null; }

    // 显示进度面板
    const panel = document.getElementById('twProgressPanel');
    if (panel) panel.style.display = 'block';
    document.getElementById('twProgressTitle').textContent = '交给大秘处理中...';
    document.getElementById('twProgressTime').textContent = new Date().toLocaleTimeString('zh-CN', {hour:'2-digit', minute:'2-digit'});

    // 渲染 Agent 列表（全部等待中）
    const demoAgents = [
      { id: 'dami', task: '拆解任务、搜资料、撰写报告' },
      { id: 'accountant', task: '需要时做数据分析' },
      { id: 'writer', task: '需要时润色文案' }
    ];
    const agentList = document.getElementById('twAgentList');
    agentList.innerHTML = demoAgents.map(a => {
      const role = ROLES[a.id];
      return `<div class="tw-agent-row">
        <img class="tw-agent-avatar" src="avatar://${a.id}.png" onerror="this.style.display='none'" />
        <div class="tw-agent-info">
          <div class="tw-agent-name">${role ? role.name : a.id}</div>
          <div class="tw-agent-task">${a.task}</div>
        </div>
        <span class="tw-agent-status waiting">⏳ 等待中</span>
      </div>`;
    }).join('');
