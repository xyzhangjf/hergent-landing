  ...
  function loadSkills() {
    ...
    if (!grid) return;
    try {
      const roleIcons = { dami:'🤵', accountant:'💰', programmer:'💻', writer:'✍️', screenwriter:'🎬', tutor:'📚', health:'💚', investor:'📈' };
      const categories = Object.entries(ROLES).map(([id, r]) => {
        const scenes = ROLE_SCENES[id] || [];
        return {
          icon: roleIcons[id] || '📦',
          name: r.name,
          description: `共 ${scenes.length} 项技能`,
          skills: scenes.map(s => ({ name: `${s.icon} ${s.text}`, description: s.prompt }))
        };
      }).filter(c => c.skills.length > 0);

      // 内置技能（来自 Hermes Agent 原生能力）
      categories.push({
        icon: '🧠',
        name: '编程方法论',
        description: 'Karpathy 的 LLM 编程最佳实践',
        skills: [
          { name: 'Karpathy Coding Guide', description: '先想再写 · 简单优先 · 精准修改 · 目标驱动 — 提升 AI 编码质量的 4 条核心规则' },
        ]
      });
