...
async function startHermesGateway() {
  ...
  if (isWindows) {
    ...
    const pythonBin = path.join(path.dirname(HERMES_BIN), 'python', 'bin', 'python3.11');
...
function ensureEngineConfig() {
  ...
  try {
    ...
    if (existing !== expectedConfig) {
      fs.writeFileSync(configPath, expectedConfig);
    }
  } catch (_) {}
}

// 每次启动确保内置技能就位（写入 Hermes skills 目录，Agent 可按需加载）
function ensureBuiltinSkills() {
  const engineDir = getEngineDir();
  const skillsDir = path.join(engineDir, '.hermes', 'skills', 'karpathy-coding');
...
function resolveHermesPath() {
  ...
  if (extractBundledEngine()) {
    ensureEngineConfig(); // 每次启动确保 config 正确
    ensureBuiltinSkills(); // 每次启动确保内置技能就位
    const engineDir = getEngineDir();
...
`);
      ...
      if (action === 'chat' || !action) {
          ...
          if (isWindows) {
            ...
            try {
              const child = spawn(winPython, [winHermes, 'chat', '-q', fullText, '--max-turns', '60', '--source', 'tool'], {
                env: { ...process.env, PYTHONPATH: path.join(engineDir, 'libs'), PYTHONHOME: '', HERMES_HOME: path.join(engineDir, '.hermes') }
              });
              ...
              try {
                await httpPost(`${SERVER_URL}/api/credits/deduct?device_id=${getDeviceId()}`,
                  JSON.stringify({ credits: cliCreditsUsed, model: 'hermes' }),
                  { timeout: 5000 }
                );
              } catch (de) {
                fs.appendFileSync(logFile, `[${new Date().toISOString()}] deduct failed: ${de.message}\n`);
              }
              return { requestId, success: true, output: responseText.slice(0, 8000), offline: true, cost: cliCreditsUsed };
            } catch (e) {
        ...
        try {
          ...
          // 扣减积分（失败不阻塞回复）
          const gwCreditsUsed = Math.max(1, Math.ceil((fullText.length + gwResponseText.length) / 500));
          try {
            await httpPost(`${SERVER_URL}/api/credits/deduct?device_id=${getDeviceId()}`,
              JSON.stringify({ credits: gwCreditsUsed, model: 'hermes' }),
              { timeout: 5000 }
            );
          } catch (de) {
            fs.appendFileSync(logFile, `[${new Date().toISOString()}] deduct failed: ${de.message}\n`);
          }
          return { requestId, success: true, output: gwResponseText, offline: true, cost: gwCreditsUsed };
        } catch (e) {
          return { requestId, success: false, output: `执行失败：${e.message}` };
        }
      }
