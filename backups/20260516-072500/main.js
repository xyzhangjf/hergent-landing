...
const SERVER_URL = (() => {
  ...
  return 'https://api.hergent.cn';  // 正式域名（配好 DNS + SSL 后生效）
...
// ===== IPC: 充值积分 =====
const ADMIN_SECRET = 'fc6c1452518ffca4bf7aaf16ac6079a34c2d8578f33e11eb443343a58b055fa9';

ipcMain.handle('recharge:request', async (event, amount) => {
  try {
    const deviceId = getDeviceId();
    const payload = JSON.stringify({ amount: amount, device_id: deviceId });
    const body = await httpPost(`${SERVER_URL}/api/recharge`, payload, {
    ...
    if (e.message && e.message.startsWith('{')) {
      try { return JSON.parse(e.message); } catch {}
    }
    return { success: false, message: e.message || '充值失败，请稍后重试' };
  }
});
...
ipcMain.handle('usage:history', async (event, limit) => {
  try {
    const body = await httpGet(`${SERVER_URL}/api/usage/history?limit=${limit || 20}`);
    return JSON.parse(body);
  } catch (e) {
...
app.whenReady().then(() => {
  ...
  // ===== 自动更新 =====
  autoUpdater.autoDownload = false;  // 手动控制，先通知用户
  ...
  autoUpdater.on('download-progress', (info) => {
    mainWindow?.webContents.send('update:status', { status: 'downloading', percent: info.percent });
  });
